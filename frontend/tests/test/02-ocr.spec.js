...
describe ('OCR', function() {
  ...
  it('returns ocr results', async function() {
    // select file
    ...
    const toast = await driver.wait(until.elementLocated(By.css(".add-invoice-success-toast")), 40000);
    await driver.wait(async () => (await toast.getText()).trim() !== '', 5000);
    assert.strictEqual(await toast.getText(), 'Invoice processed successfully!');

    // process file completed when store-file-button is rendered
    await driver.wait(until.elementLocated(By.id('store-file-button')), 30000);

    // assert text in process-file-button shows 'Image Processed'
    assert.strictEqual(await driver.findElement(By.id('process-file-button')).getText(), 'Image Processed');

    // ocr should return at least one of the values
    const invoiceId = await driver.findElement(By.css('[data-testid="invoice-id-input"]'));
    const totalPriceBeforeGST = await driver.findElement(By.css('[data-testid="total-price-before-gst-input"]'));
    const totalPriceAfterGST = await driver.findElement(By.css('[data-testid="total-price-after-gst-input"]'));

    assert.ok(invoiceId || totalPriceBeforeGST || totalPriceAfterGST);
  })
})